...
class controlNode(Node):
    ...
    def timer_callback(self):
        ...
        px,py,pz = 0.4,0.4,1.0
        roll,pitch,yaw = 1, -1, -1
        q1,q2,q3,q4,q5,q6 = self.get_angles(px,py,pz,-1,-1,-1)
        #self.theta1 = np.radians(q1)
        #self.theta2 = np.radians(q2)
        #self.theta3 = np.radians(q3)
        #self.theta4 = np.radians(q4)
        #self.theta5 = np.radians(q5)
        #self.theta6 = np.radians(q6)
        q1=float(q1)
        q1=round(q1,1)
        q2=float(q2)
        q2=round(q2,1)
...
#---------------------------------------
  ...
  # calculate angle gamma between sides a and b using cosine law
    ...
    def get_wrist_center(self,x,y,z, R0g1, dg = 0.303):
  ...
  # check WRITEUP.pdf for more info
     ...
     #print(y)
     #print(z)
     nx, ny, nz = R0g1[0, 2], R0g1[1, 2], R0g1[2, 2]
     xw = xu - dg * nx
     yw = yu - dg * ny
     zw = zu - dg * nz 
  ...
  # Inverse kinematics transformations starts below
     ...
     wrist_center = self.get_wrist_center(x,y,z, R0g_eval)
